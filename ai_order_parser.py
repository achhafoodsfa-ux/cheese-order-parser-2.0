...
SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        ...
        "items": {"type": "array", "items": {"type": "object", "properties": {"raw_text": {"type": "string"}, "product": {"type": "string"}, "quantity": {"type": "number"}, "unit": {"type": "string", "enum": ["CTN", "PKT", "KG", "PCS"]}}, "required": ["raw_text", "product", "quantity", "unit"], "additionalProperties": False}}
    },
...
- Preserve quantities and units exactly when clearly stated. pkt/packet/packets/pcs/pc/units = PKT/PCS as appropriate; kg = KG.
...
def _fallback_text_result(text: str, reason: str) -> Dict[str, Any]:
    return {"customer_name": "", "items": [], "_fallback": True, "_fallback_text": text, "_fallback_reason": reason}
...
def _fallback_image_result(image_bytes: bytes, reason: str) -> Dict[str, Any]:
    try:
        ...
        lines = [" ".join(raw.split()).strip() for raw in text.replace("\x00", " ").splitlines() if " ".join(raw.split()).strip()]
...
def ai_parse_order_text(text: str) -> Dict[str, Any]:
    return _call_ai(lambda: _parse_response(_client().responses.create(model=MODEL, store=False, instructions=INSTRUCTIONS, input=text, text={"format": {"type": "json_schema", "name": "cheese_order", "schema": SCHEMA, "strict": True}})), lambda reason: _fallback_text_result(text, reason))

def ai_parse_order_image(image_bytes: bytes) -> Dict[str, Any]:
    data_url = "data:image/png;base64," + base64.b64encode(image_bytes).decode("ascii")
    return _call_ai(lambda: _parse_response(_client().responses.create(model=MODEL, store=False, instructions=INSTRUCTIONS, input=[{"role": "user", "content": [{"type": "input_text", "text": "Read this order screenshot and extract the structured order."}, {"type": "input_image", "image_url": data_url, "detail": "high"}]}], text={"format": {"type": "json_schema", "name": "cheese_order", "schema": SCHEMA, "strict": True}})), lambda reason: _fallback_image_result(image_bytes, reason))
...
def ai_to_parser_text(result: Dict[str, Any]) -> str:
    ...
    for item in result.get("items", []):
        qty = item.get("quantity"); unit = item.get("unit", "PKT"); product = item.get("product", "")
